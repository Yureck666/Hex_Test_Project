...
{
    #if !SOFTMASK_EDITOR
    if (_AllowRenderScale > 0.5)
    {
        uv /= _RenderScale;
    }
    if (_AllowDynamicResolution > 0.5)
    {
    ...
    #if SOFTMASK_EDITOR
    float inScreen = step(0.0, uv.x) * step(uv.x, 1.0) * step(0.0, uv.y) * step(uv.y, 1.0);
    alpha = lerp(_SoftMaskOutsideColor, alpha, inScreen);
    #ifdef UNITY_UI_ALPHACLIP
    if (_MaskingShapeSubtract > 0.5)
    {
